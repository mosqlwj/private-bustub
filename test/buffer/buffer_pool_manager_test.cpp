...
TEST(BufferPoolManagerTest, FetchPage) {
  ...
  // 首先10个newpage（来自freelist），并且写入内容
  for (int i = 0; i < 10; ++i) {
    auto new_page = bpm->NewPage(&temp_page_id);
    ASSERT_NE(nullptr, new_page);
    strcpy(new_page->GetData(), std::to_string(i).c_str());  // NOLINT
...
TEST(BufferPoolManagerTest, IsDirty) {
  ...
  strcpy(page0->GetData(), "page0");  // NOLINT
  // 总算知道unpin这个api的意义了：我用完了，然后我再传一个参数is_dirty,说明我又没有修改这个page.这里就是说我修改了
  EXPECT_EQ(1, bpm->UnpinPage(pageid0, true));
...
TEST(BufferPoolManagerConcurrencyTest, HardTest_1) {
  ...
  std::vector<page_id_t> page_ids;
  // 每次new
  // page10个，都向里面写点东西，然后把其中前面5个unpin的时候，isDirty设为false，后面5个unpin的时候，isDirty设为true
  // 总共重复1000轮，所以总共new了10000个page，因为他们都被unpin了，所以也都被加入到replacer了
  for (int j = 0; j < 1000; j++) {
    for (int i = 0; i < 10; i++) {
      auto new_page = bpm->NewPage(&temp_page_id);
      EXPECT_NE(nullptr, new_page);
